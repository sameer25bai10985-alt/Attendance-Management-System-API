...
## 1. Flask Blueprints over a single monolithic file
...
lets each blueprint be registered/unregistered independently in `app.py`.
...
layer can be tested or reused (e.g. from a CLI script) without going through Flask at all.
...
mobile, or another service), rather than tying the logic to a specific UI.
